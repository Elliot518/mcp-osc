...
import com.mcp.osc.infrastructure.util.regex.Validators;
...
public abstract class DateTimeUtils {
     ...
     * @return
     */
    public static boolean isDate(String date, String sdf) {
    ...
    public static LocalDateTime strToLocalDate(String timeStr) {
        String standTime = timeStr.replaceAll("/", "-");
        LocalDateTime localDateTime = null;
        try {
            Date date = DateUtils.parseDate(standTime, "yyyy-MM-dd");
    ...
    public static LocalDateTime strToLocalDateTime(String timeStr) {
        if ("".equals(timeStr.trim())) {
            return null;
        }

        String standTime = timeStr.replaceAll("/", "-");

        if (standTime.length() <= 10) {
            standTime = standTime + " 00:00:00";
        }

        Date date = string2DateTime(standTime);

        return date.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
    }

    public static LocalDateTime strToLocalDateTime2(String timeStr) {
        if ("".equals(timeStr.trim())) {
            return null;
        }

        Date date = string2Date(timeStr, "yyyyMMdd");

        return date.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
    }

    public static LocalDateTime strToLocalDateTime(String timeStr, String dateFormat) {
        DateTimeFormatter timeDtf = DateTimeFormatter.ofPattern(dateFormat);
    ...
    public static long dateDiffByDays(LocalDateTime localDate1, LocalDateTime localDate2) {
        if (localDate1 == null || localDate2 == null) {
            return 0;
        }

        return localDate1.toLocalDate().toEpochDay() - localDate2.toLocalDate().toEpochDay();
    }

    public static void main(String[] args) {
        String strDate = "2016-10-1 00:00:00";
        System.out.println(strToLocalDateTime(strDate));
    }
}
